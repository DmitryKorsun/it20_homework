...
SELECT MIN(`count_students`) AS 'Минимум' FROM `organizations` WHERE `type_id` = 5;
/* 6. Посчитать сумму оценок rating из таблицы practice, где
practice_id = 1888. */
SELECT SUM(`rating`) AS 'Сумма оценок' FROM `practice` WHERE `practice_id` = 1888;
/* 7. Подсчитать количество записей в таблице organizations. */
SELECT COUNT(*) AS 'Количество записей' FROM `organizations`;
/* 8. Необходимо вывести 10 новостей из таблицы news, отсортировать
по дате добавления created_at. */
SELECT * FROM `news` ORDER BY `created_at` ASC LIMIT 10
...
SELECT `phone`,`name` FROM `directors` WHERE `organization` LIKE 'ВУЗ%'
...
(body). */
SELECT `publish_date` AS 'дата',`body` AS 'текст' FROM `publishes` LIMIT 5;
...
SELECT `region_id`,COUNT(*) AS 'Количество Школ' FROM `schools` GROUP BY `region_id`;